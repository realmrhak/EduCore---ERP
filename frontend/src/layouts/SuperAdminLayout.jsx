...
const navItems = [
  { label: 'Dashboard', icon: LayoutDashboard, path: '/superadmin/dashboard', roles: ['superadmin'] },
  { label: 'Students', icon: Users, path: '/superadmin/students', roles: ['superadmin'] },
  { label: 'Classes', icon: Layers, path: '/superadmin/classes', roles: ['superadmin'] },
  { label: 'Student Profiles', icon: Users, path: '/superadmin/attendance', roles: ['superadmin'] },
  { label: 'Timetable', icon: Clock, path: '/superadmin/timetable', roles: ['superadmin'] },
  { label: 'Exams', icon: FileText, path: '/superadmin/exams', roles: ['superadmin'] },
  { label: 'Quizzes', icon: ClipboardList, path: '/superadmin/quizzes', roles: ['superadmin'] },
  { label: 'Results', icon: FileText, path: '/superadmin/results', roles: ['superadmin'] },
  { label: 'Reports', icon: BarChart3, path: '/superadmin/reports', roles: ['superadmin'] },
  { label: 'Users', icon: Users, path: '/superadmin/users', roles: ['superadmin'] },
  { label: 'Departments', icon: Building2, path: '/superadmin/departments', roles: ['superadmin'] },
  { label: 'Subjects', icon: BookOpen, path: '/superadmin/subjects', roles: ['superadmin'] },
  { label: 'Fee Challans', icon: CreditCard, path: '/superadmin/challans', roles: ['superadmin'] },
  { label: 'Fee Structure', icon: CreditCard, path: '/superadmin/fee-structure', roles: ['superadmin'] },
  { label: 'Library', icon: BookOpen, path: '/superadmin/library', roles: ['superadmin'] },
  { label: 'Notifications', icon: Bell, path: '/superadmin/notifications', roles: ['superadmin'] },
  { label: 'Activity Logs', icon: BarChart3, path: '/superadmin/activity-logs', roles: ['superadmin'] },
  { label: 'Settings', icon: Settings, path: '/superadmin/settings', roles: ['superadmin'] },
];
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex">
      ...
      `}>
        ...
        <div className={`h-16 flex items-center gap-3 px-4 border-b border-[#E2E8F0] ${!sidebarOpen && 'lg:justify-center'}`}>
          ...
          {(sidebarOpen || mobileOpen) && (
            <div className="overflow-hidden">
              ...
              <p className="text-[10px] text-[#94A3B8] uppercase tracking-wider whitespace-nowrap">Super Admin Portal</p>
            </div>
          )}
        ...
        <div className="p-3 border-t border-[#E2E8F0] space-y-1">
          <button onClick={() => navigate('/superadmin/help')} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-[#475569] hover:bg-[rgba(22,163,74,0.04)] hover:text-[#16a34a] transition-all">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-14 sm:h-[68px] bg-white border-b border-[#E2E8F0] flex items-center justify-between px-3 sm:px-6 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <button onClick={() => navigate('/superadmin/profile')} className="p-2 rounded-lg hover:bg-[#F1F5F9] transition-colors hidden sm:flex" title="Profile">
              <Settings className="w-5 h-5 text-[#475569]" />
            </button>
            <button onClick={() => navigate('/superadmin/profile')} className="flex items-center gap-2 ml-1 sm:ml-2 pl-2 sm:pl-3 border-l border-[#E2E8F0] hover:opacity-80">
